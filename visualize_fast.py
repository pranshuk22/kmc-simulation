# visualize.py

import numpy as np
import matplotlib.pyplot as plt
from matplotlib.widgets import Slider
import sys
import os
import json     # Uses json.loads, much faster than ast
# import ast    <- No longer needed

class LatticePlot:
    """
    Visualizer for the KMC simulation.
    --- OPTIMIZED ---
    Uses a scatter plot for particle markers for faster updates.
    """
    def __init__(self, data):
        self.lattice_size = data['lattice_size']
        self.num_stripes = data['num_stripes']
        self.snapshots = data['snapshots']
        self.time_stamps = data['time_stamps']
        self.stripe_rates = data.get('stripe_rates', [])
        
        plt.style.use('seaborn-v0_8-darkgrid')
        self.fig, self.ax = plt.subplots(figsize=(10, 8))
        self.fig.suptitle("Interactive Kinetic Monte Carlo Simulation", fontsize=16, weight='bold')
        plt.subplots_adjust(bottom=0.2, top=0.85)
        
        self._configure_axes()
        self._draw_background_stripes()
        
        # --- NEW: Use a scatter plot for the particle "body" ---
        self.particle_scatter = self.ax.scatter([], [], s=80, c='white', ec='black', zorder=3, alpha=0.7)
        
        self.particle_texts = []
        
        self.info_text = self.ax.text(0.02, 0.98, '', transform=self.ax.transAxes,
                                      fontsize=12, family='monospace', va='top', ha='left',
                                      bbox=dict(boxstyle='round,pad=0.3', fc='wheat', alpha=0.8))
        
        self._create_slider()

    def _configure_axes(self):
        self.ax.set_xlabel("X-coordinate")
        self.ax.set_ylabel("Y-coordinate")
        self.ax.set_xlim(-0.5, self.lattice_size - 0.5)
        self.ax.set_ylim(-0.5, self.lattice_size - 0.5)
        self.ax.set_aspect('equal')

    def _draw_background_stripes(self):
        stripe_width = self.lattice_size / self.num_stripes
        stripe_colors = ['#d6e8ee', '#f5dcdc']
        for i in range(self.num_stripes):
            rate = self.stripe_rates[i]
            rate_text = f"Fast ({rate})" if rate > 0.1 else f"Slow ({rate})"
            x_start = i * stripe_width
            self.ax.add_patch(
                plt.Rectangle((x_start - 0.5, -0.5), stripe_width, self.lattice_size,
                              color=stripe_colors[i % 2], alpha=0.5, zorder=0)
            )
            self.ax.text(x_start + stripe_width/2 - 0.5, self.lattice_size + 0.5,
                         rate_text, ha='center', va='bottom', fontsize=9)
            if i > 0:
                self.ax.axvline(x=x_start - 0.5, color='gray', linestyle='--')

    def _create_slider(self):
        ax_slider = plt.axes([0.15, 0.05, 0.7, 0.03], facecolor='lightgoldenrodyellow')
        self.slider = Slider(
            ax=ax_slider,
            label='Time (s)',
            valmin=0.0,
            valmax=self.time_stamps[-1],
            valinit=0.0,
            valfmt='%.2f s'
        )
        self.slider.on_changed(self.update)

    def update(self, val):
        """--- OPTIMIZED update function ---"""
        time = self.slider.val
        step_idx = np.searchsorted(self.time_stamps, time, side="right") - 1
        current_snapshot = self.snapshots[step_idx]

        # 1. Clear the old text labels
        for txt in self.particle_texts:
            txt.remove()
        self.particle_texts.clear()
        
        # Get all current particle positions
        positions = list(current_snapshot.values())
        
        if not positions:
            # No particles, clear scatter and text
            self.particle_scatter.set_offsets(np.empty((0, 2)))
        else:
            # 2. Update scatter plot (fast, vectorized)
            self.particle_scatter.set_offsets(positions)
            
            # 3. Draw new text labels (slower, but necessary for PIDs)
            for pid_str, (x, y) in current_snapshot.items():
                # pid is a string from JSON, but that's fine
                txt = self.ax.text(x, y, pid_str, ha='center', va='center',
                                   color='black', fontsize=8, weight='bold', zorder=4)
                self.particle_texts.append(txt)
        
        # 4. Update info text box
        self.info_text.set_text(f"Time: {time:.2f} s\nParticles: {len(current_snapshot)}")
        
        self.fig.canvas.draw_idle()

    def show(self):
        self.update(0)
        plt.show()

# --- HEAVILY MODIFIED: No longer computes, just plots ---
def plot_particle_count(data):
    """
    Generates plots from PRE-CALCULATED data.
    """
    # --- Extract PRE-CALCULATED data ---
    time_stamps = data['time_stamps']
    total_particle_counts = data['total_particle_counts']
    stripe_counts = data['stripe_counts']
    num_stripes = data['num_stripes']
    
    plt.style.use('seaborn-v0_8-whitegrid')

    # --- Plot 1: Total System Occupancy ---
    fig1, ax1 = plt.subplots(figsize=(10, 5))
    avg_count = np.mean(total_particle_counts)
    
    ax1.plot(time_stamps, total_particle_counts, lw=2, color="teal", label="Total Particle Count")
    ax1.axhline(avg_count, color="crimson", linestyle="--", lw=2, label=f"Avg = {avg_count:.2f}")
    
    ax1.set_title("Total System Occupancy Over Time", fontsize=16)
    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Number of Particles")
    ax1.legend()
    ax1.grid(True, which='both', linestyle='--', linewidth=0.5)
    plt.tight_layout()
    plt.show()

    # --- Plot 2: Occupancy per Stripe ---
    if num_stripes > 1:
        fig2, axes2 = plt.subplots(
            nrows=num_stripes, 
            ncols=1, 
            figsize=(10, 2.5 * num_stripes), 
            sharex=True
        )
        fig2.suptitle("Particle Occupancy per Stripe", fontsize=16, y=0.95)

        for i in range(num_stripes):
            ax = axes2[i]
            counts = stripe_counts[i] # Get the pre-calculated list
            avg_count_stripe = np.mean(counts)
            
            ax.plot(time_stamps, counts, lw=2, label=f"Stripe {i+1} Count")
            ax.axhline(avg_count_stripe, color="crimson", linestyle="--", lw=2, label=f"Avg = {avg_count_stripe:.2f}")
            ax.set_ylabel("Particle Count")
            ax.legend(loc='upper right')
            ax.grid(True, which='both', linestyle='--', linewidth=0.5)

        axes2[-1].set_xlabel("Time (s)")
        plt.tight_layout(rect=[0, 0, 1, 0.93])
        plt.show()


# --- NEW FUNCTION: Replaces parse_snapshot_file ---
def load_and_process_data(config_filename, snapshot_filename):
    """
    Loads config and parses the snapshot file (JSONL),
    processing/aggregating data on the fly for speed.
    """
    print("✅ Loading config data...")
    try:
        with open(config_filename, 'r') as f:
            data = json.load(f)
    except Exception as e:
        print(f"❌ Error loading '{config_filename}': {e}")
        sys.exit(1)

    # Get metadata needed for processing
    lattice_size = data['lattice_size']
    num_stripes = data['num_stripes']
    stripe_width = lattice_size // num_stripes

    print(f"Parsing and processing '{snapshot_filename}'... This is now much faster!")
    
    # Prepare lists to hold all data
    snapshots = []
    time_stamps = []
    total_particle_counts = []
    stripe_counts = [[] for _ in range(num_stripes)]

    try:
        with open(snapshot_filename, 'r') as f:
            for line in f:
                if not line.strip():
                    continue
                
                # 1. Parse the JSON line (much faster than ast)
                record = json.loads(line)
                
                time_val = record['t']
                snapshot_dict = record['snapshot']
                
                # 2. Add raw data to lists for interactive plot
                time_stamps.append(time_val)
                snapshots.append(snapshot_dict)
                
                # 3. --- PROCESS DATA ON THE FLY ---
                # This solves the aggregation bottleneck!
                total_particle_counts.append(len(snapshot_dict))
                
                counts_in_snapshot = [0] * num_stripes
                for pos_str in snapshot_dict.values():
                    # pos_str is [x, y] from JSON, not a tuple
                    x = pos_str[0] 
                    stripe_index = min(x // stripe_width, num_stripes - 1)
                    counts_in_snapshot[stripe_index] += 1
                
                for i in range(num_stripes):
                    stripe_counts[i].append(counts_in_snapshot[i])

    except Exception as e:
        print(f"❌ Error parsing snapshot file: {e}")
        print("➡️ Make sure you re-ran 'run_simulation_fast.py' to generate the new JSONL snapshot format.")
        sys.exit(1)
        
    print(f"Successfully parsed and processed {len(snapshots)} snapshots.")
    
    # Add aggregated data back into the main dictionary
    data['snapshots'] = snapshots
    data['time_stamps'] = time_stamps
    data['total_particle_counts'] = total_particle_counts
    data['stripe_counts'] = stripe_counts
    
    return data


# --- UPDATED MAIN BLOCK ---
if __name__ == "__main__":
    CONFIG_FILENAME = "simulation_config.json"
    SNAPSHOT_FILENAME = "snapshots.txt"
    
    if not os.path.exists(CONFIG_FILENAME) or not os.path.exists(SNAPSHOT_FILENAME):
        print(f"❌ Error: Missing required files.")
        print(f"   - Looking for: '{CONFIG_FILENAME}'")
        print(f"   - Looking for: '{SNAPSHOT_FILENAME}'")
        print("➡️ Please run 'python run_simulation_fast.py' first to generate the new data files.")
        sys.exit(1)
        
    # This one function now does all the heavy loading and processing
    simulation_data = load_and_process_data(CONFIG_FILENAME, SNAPSHOT_FILENAME)

    # Launch the interactive plot
    plot = LatticePlot(simulation_data)
    plot.show()
    
    # Launch the particle count plots (which are now very fast)
    plot_particle_count(simulation_data)